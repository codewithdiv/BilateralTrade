            ...
            <v-dialog v-model="dialog" persistent max-width="600px">
              <template v-slot:activator="{ on, attrs }">
                <v-btn
                  ...
                  v-on="on"
                >
                  <v-icon>mdi-text-box-plus</v-icon>Upload Documents
                </v-btn>
              </template>
              <v-card>
                <v-card-title>
                  <span v-show="!editmode" class="headline">Upload Document</span>
                  <span v-show="editmode" class="headline">Edit Document</span>
                </v-card-title>
                <v-card-text>
                  <v-container>
                    <v-row>
                      ...
                      <v-col cols="12" sm="12" md="6" v-show="!editmode">
                        <v-autocomplete
                          :items="['Pending', 'Ongoing', 'Approved']"
                          v-model="data.status"
                    ...
                    <v-col cols="12" v-show="!editmode">
                        <v-textarea v-model="data.remarks" outlined color="green darken-4">
                        ...
                        >
                          <div style="padding: 20px 0">
                            <Icon
                              ...
                              style="color #3399ff"
                            ></Icon>
                            <p>Click or drag here to upload document</p>
                          </div>
                        </Upload>

                        <div class="demo-upload-list" v-if="data.document">
                    ...
                    <v-col cols="12" v-show="editmode">
                        <v-textarea v-model="editData.remarks" outlined color="green darken-4">
                            <template v-slot:label>
                                <div>
                                    Document Remark
                                    <small
                      ...
                      <v-col v-show="editmode" cols="12" sm="12" md="12">
                        ...
                        <div class="demo-upload-list" v-if="editData.document">
                          <img :src="`${editData.document}`" />
                          <div
                            class="demo-upload-list-cover"
                            v-show="!editmode"
                          >
                ...
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn color="error darken-4" v-show="!editmode" dark @click="dialog = false">
                    Close
                  </v-btn>
                  <v-btn v-show="editmode" color="error darken-4" dark @click="closeDocumentDialog">
                    Close
                  </v-btn>
                  <v-btn
                    color="success darken-4"
                    @click.prevent="
                      editmode ? updateDocument() : addDocument();
                      loader = 'isAdding';
                    "
                    :loading="isAdding"
                    :disabled="isAdding"
          ...
          <div class="_overflow _table_div">
            <table class="_table">
              ...
              <tr v-for="(document, i) in documents" :key="i">
                ...
                <td class="uppercase">{{ document.country }}</td>
                <td class="uppercase">{{ document.subject }}</td>
